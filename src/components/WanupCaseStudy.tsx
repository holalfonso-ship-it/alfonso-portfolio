...
// ─── Image imports ────────────────────────────────────────────────────────────
import heroImg from '@/assets/1er-wanup.png';
import screen2Img from '@/assets/2nd-wanup.png';
import screen3Img from '@/assets/3rd-wanup.png';
import screen4Img from '@/assets/4to-wanup.png';
import screen5Img from '@/assets/5to-wanup.png';
  ...
  return (
    <section id="case-study-wanup" className="py-28 md:py-36 bg-background">
      ...
      <div ref={headerReveal.ref} className={`px-6 md:px-12 max-w-7xl mx-auto mb-14 md:mb-20 ${headerReveal.className}`}>
        <span className="text-sm font-medium tracking-wider py-1 px-4 rounded-full bg-primary/10 text-primary w-fit inline-block mb-6">Case Study · B2C</span>
        <h2 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold tracking-tight max-w-4xl leading-[1.05] mb-6">
          <AnimatedText text="Wanup: Redesigning the Hotel Booking Experience" className="inline-block" once />
        </h2>
      ...
      <div className="w-full px-0 md:px-6 lg:px-12 max-w-7xl mx-auto mb-0">
        <div className="relative w-full overflow-hidden rounded-none md:rounded-2xl bg-secondary/40" style={{ aspectRatio: '16/9' }}>
          <img src={heroImg} alt="Wanup hero" className="w-full h-full object-cover object-center" />
      ...
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <div className="max-w-3xl mx-auto pt-16 md:pt-24 pb-4 text-center">
          <p className="text-xl md:text-2xl text-muted-foreground leading-relaxed">
            ...
            <span className="font-semibold text-foreground">Wanup</span> set out to fix that — and add a loyalty layer that actually makes users come back.
          </p>
        </div>
      </div>
      ...
      {/* ── IMAGE 2 */}
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <FullImage src={screen2Img} alt="Hotel detail page — availability, rooms and booking widget" caption="The redesigned hotel detail page — availability, room options, and the booking CTA are all visible above the fold, eliminating the need to scroll before deciding." />
      </div>
      ...
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={situation.ref} className={`py-16 md:py-24 border-t border-border/20 ${situation.className}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr_5fr] gap-8 md:gap-20 items-start">
            <h3 className="text-3xl md:text-4xl font-display font-bold tracking-tight leading-tight">A Broken Funnel &amp; Zero Retention</h3>
            <div className="space-y-4">
              <p className="text-muted-foreground text-lg md:text-xl leading-relaxed">The original platform had a <span className="font-semibold text-foreground">61% abandonment rate</span> at the hotel detail page — users couldn't find availability without navigating through three separate screens, and the checkout process had 11 form steps with no progress indicator.</p>
              <p className="text-muted-foreground text-lg md:text-xl leading-relaxed">Beyond acquisition, there was <span className="font-semibold text-foreground">no retention strategy</span>: no loyalty programme, no saved favourites, no post-booking engagement. Every user was effectively a first-time user, regardless of how many times they had booked before.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── IMAGE 3 */}
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <FullImage src={screen3Img} alt="Checkout flow — streamlined form and progress indicator" caption="Checkout redesign — 11 steps collapsed into 3 stages with a persistent progress indicator. Form fields reduced from 18 to 11 by removing non-essential data collection." />
      </div>
      ...
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={task.ref} className={`py-16 md:py-24 border-t border-border/20 ${task.className}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr_5fr] gap-8 md:gap-20 items-start">
            <h3 className="text-3xl md:text-4xl font-display font-bold tracking-tight leading-tight">Convert Better. Keep Users Longer.</h3>
            <div className="space-y-4">
              <p className="text-muted-foreground text-lg md:text-xl leading-relaxed">The brief had two distinct goals: first, reduce friction in the booking funnel to improve conversion — specifically targeting the hotel detail page and checkout completion rate. Second, design a <span className="font-semibold text-foreground">loyalty and retention system</span> that gave users a reason to return to Wanup instead of switching to a competitor for their next stay.</p>
              <p className="text-muted-foreground text-lg md:text-xl leading-relaxed">Both goals required a <span className="font-semibold text-foreground">scalable Design System</span> — the existing UI had no shared components, making every new screen a from-scratch effort that slowed the team and created visual inconsistency across the product.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── IMAGE 4 */}
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <FullImage src={screen4Img} alt="Cash Rewards and Member Get Member loyalty dashboard" caption="Cash Rewards dashboard and Member Get Member flow — users earn credit on every booking and unlock referral bonuses, creating a flywheel that drives both retention and organic acquisition." />
      </div>
      ...
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={action.ref} className={`py-16 md:py-24 border-t border-border/20 ${action.className}`}>
          ...
          <h3 className="text-3xl md:text-4xl font-display font-bold tracking-tight mb-16 max-w-xl">Funnel First, Loyalty Second, System Always</h3>
      ...
      {/* ── IMAGE 5 */}
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <FullImage src={screen5Img} alt="Wanup design system — component library and token structure" caption="Wanup Design System — a unified component library spanning hotel cards, booking widgets, forms, loyalty UI, and user account screens. Built with tokenised variables for consistent theming." />
      </div>
      ...
      <div className="px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={impact.ref} className={`py-16 md:py-24 border-t border-border/20 ${impact.className}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr_5fr] gap-8 md:gap-20 items-start mb-16">
            <h3 className="text-3xl md:text-4xl font-display font-bold tracking-tight leading-tight">Measurable Conversion &amp; Retention Gains</h3>
            <p className="text-muted-foreground text-lg md:text-xl leading-relaxed">Results were tracked across a 90-day post-launch window covering both conversion metrics from the redesigned funnel and retention data from the new loyalty layer — measured against the same cohort from the previous quarter.</p>
          </div>
            ...
            {[
              ...
              <div key={category.title}>
                ...
                <div className="flex flex-wrap gap-2">
                  {category.pills.map((pill) => (<span key={pill} className="px-4 py-1.5 text-sm rounded-full bg-secondary/60 text-foreground/80 border border-border/30">{pill}</span>))}
                </div>
              </div>
            ))}
          </div>
